...
public class ShipmentServiceImpl implements ShipmentService {
	...
	public ShipmentResponseDTO createShipment(ShipmentRequestDTO shipmentRequestDTO) {
		...
		Shipment shipment = Shipment.builder().weight(shipmentRequestDTO.getWeight())
				.budget(shipmentRequestDTO.getBudget()).status(shipmentRequestDTO.getStatus())
				.sourceLocation(sourceLocation).destinationLocation(destinationLocation).shipper(shipper).build();

		Shipment savedShipment = shipmentRepository.save(shipment);

		return ShipmentResponseDTO.builder().id(savedShipment.getId()).weight(savedShipment.getWeight())
				.budget(savedShipment.getBudget()).status(savedShipment.getStatus())
				.sourceLocationId(savedShipment.getSourceLocation().getId())
				.destinationLocationId(savedShipment.getDestinationLocation().getId())
				.shipperId(savedShipment.getShipper().getId()).build();
	}
	...
	public ShipmentResponseDTO getShipmentById(Long id) {
		...
		return ShipmentResponseDTO.builder().id(shipment.getId()).weight(shipment.getWeight())
				.budget(shipment.getBudget()).status(shipment.getStatus())
				.sourceLocationId(shipment.getSourceLocation().getId())
				.destinationLocationId(shipment.getDestinationLocation().getId())
				.shipperId(shipment.getShipper().getId()).build();
	}
	...
	public List<ShipmentResponseDTO> getAllShipments(ShipmentStatus status, int page, int size, String sortBy,
		...
		return shipmentPage.getContent().stream()
				.map(shipment -> ShipmentResponseDTO.builder().id(shipment.getId()).weight(shipment.getWeight())
						.budget(shipment.getBudget()).status(shipment.getStatus())
						.sourceLocationId(shipment.getSourceLocation().getId())
						.destinationLocationId(shipment.getDestinationLocation().getId())
						.shipperId(shipment.getShipper().getId()).build())
	...
	public ShipmentResponseDTO updateShipment(Long id, ShipmentRequestDTO shipmentRequestDTO) {
		...
		return ShipmentResponseDTO.builder().id(updatedShipment.getId()).weight(updatedShipment.getWeight())
				.budget(updatedShipment.getBudget()).status(updatedShipment.getStatus())
				...
				.shipperId(updatedShipment.getShipper().getId()).build();
	}
